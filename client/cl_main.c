...
static void CL_ProcessDeferredAsset (int kind, int index, int step)
{
	...
	{
	case DA_MODEL:
		if (!cl.configstrings[CS_MODELS+index][0])
			return;
		cl.model_draw[index] = re.RegisterModel (cl.configstrings[CS_MODELS+index]);
		if (cl.configstrings[CS_MODELS+index][0] == '*')
			cl.model_clip[index] = CM_InlineModel (cl.configstrings[CS_MODELS+index]);
		else
			cl.model_clip[index] = NULL;
		break;
	case DA_SOUND:
		if (!cl.configstrings[CS_SOUNDS+index][0])
			return;
		cl.sound_precache[index] = S_RegisterSound (cl.configstrings[CS_SOUNDS+index]);
		break;
	case DA_IMAGE:
		if (!cl.configstrings[CS_IMAGES+index][0])
			return;
...
//	cl_minfps = Cvar_Get ("cl_minfps", "5", 0);
	...
	Cmd_AddCommand ("writecfg", CL_WriteConfig_f);
...
Console: writeconfig / writecfg — flush config.cfg now.
=================
*/
void CL_WriteConfig_f (void)
{
