...
Useful client settings for **R1Q2v2 (Build 8038+)**. Defaults are engine defaults unless noted. This is not every cvar in the tree â€” only ones players typically want.
...
| `cl_defermodels` | `1` | Map-prep: defer non-world model loads onto the **render** frame (~1 model / 16 ms) so connect/load hitches less. Mid-game `CS_MODELS` / `CS_SOUNDS` / `CS_IMAGES` / `CS_PLAYERSKINS` updates are **always** queued the same way (net parse only stores the string; Register*/`CL_LoadClientinfo` run on the render budget). Does not touch packet send, prediction, or pmove. Timedemo still loads sync. |
