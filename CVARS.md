...
| `r_mp_visual_lean` | `1` | **BUILD 8062/8063.** When on (default), and the client is in multiplayer (`CS_MAXCLIENTS` > 1 → `r_water_alpha_ok` 2), the renderer treats expensive FX as **effective-off** without overwriting archived SP `seta` values: bloom, godrays, soft particles, world corona, light shafts, normal maps, dlight shader, dynamic lights, shadows, light corona, ambient lift, waterfog. **8063:** lean stays engaged across map changes (ClearState no longer clears `r_water_alpha_ok`); disconnect still restores full SP visuals. Singleplayer / disconnected / baseq2 local keep full visuals. Set `0` to disable the auto lean. Does **not** force `vid_restart` for MSAA — lower `gl_msaa` yourself if needed. **Archived.** Client-side only; fair-play. |
...
Build 8061/8062 Advanced Settings defaults (new installs; also `r1q2v2_visual.cfg`):
...
**Alt-tab / multi-monitor:** the client keeps sending usercmds while unfocused, using elapsed command time within the normal hitch limits. Desktop mouse movement does not control the game or set button activity. Commands preserve the current view even when the input clock has not advanced. Build 8064 removes the earlier background mouse/timing workaround: legacy Arena's warning checks repeated aim-angle reversals, not the assumed q2admin timing signature. Prefer `vid_borderless 1` over exclusive fullscreen on more than one display. Default `m_directinput 0` (Win32 mouse) is the more reliable focus-restore path; DirectInput exclusive (`m_directinput 1`) can fail to reacquire until the window is truly foreground. No extra cvar is required.
