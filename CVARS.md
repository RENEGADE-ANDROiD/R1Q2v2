...
| `r_mp_visual_lean` | `1` | **BUILD 8062.** When on (default), and the client is in multiplayer (`CS_MAXCLIENTS` > 1 → `r_water_alpha_ok` 2), the renderer treats expensive FX as **effective-off** without overwriting archived SP `seta` values: bloom, godrays, soft particles, world corona, light shafts, normal maps, dlight shader, dynamic lights, shadows, light corona, ambient lift, waterfog. Singleplayer / disconnected / baseq2 local keep full visuals. Set `0` to disable the auto lean. Does **not** force `vid_restart` for MSAA — lower `gl_msaa` yourself if needed. **Archived.** Client-side only; fair-play. |
